...
a = 1;
for r = 1:size(rep, 2)
    files = dir(['../../Data/03-05 equal' num2str(rep(r)) '/data/t*.mat']);
    ts = sort(cellfun(@(s) str2double(strrep(strrep(s, '.mat', ''), 't', '')), {files.name}));
    load(['../../Data/03-05 equal' num2str(rep(r)) '/data/t' num2str(ts(end))]);
    figure(2); clf;
    set(figure(2), 'color', 'w', 'Position', [650, 0, 220, 440]);
    a = max(a, max(10, max(max(abs(p(:, 1:3))))+2));
    col = 1-exp(log(1 - 0.85)/(a/9)*abs(p(:, 2)));
