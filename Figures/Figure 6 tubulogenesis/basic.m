clc; clear; clf; tic;
rep = [825 850 875 900 925 950 975 1000];
pic = [1 3 5 7];
stat = zeros(numel(rep), 3);
pos = [.092 .135 .89 .85];
axs = [0 0.1 0 100];
wid = .9;
subfigure(rep(pic), [46 40 36 34]);

for r = rep
    files = dir(['../../Data/03-05 equal' sprintf('%02d', r) '/data/t*.mat']);
    ts = sort(cellfun(@(s) str2double(strrep(strrep(s, '.mat', ''), 't', '')), {files.name}));
    load(['../../Data/03-05 equal' sprintf('%02d', r) '/data/t' num2str(ts(end))]);
