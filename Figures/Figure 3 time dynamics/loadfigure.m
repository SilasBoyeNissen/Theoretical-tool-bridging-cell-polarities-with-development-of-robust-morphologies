function loadfigure(folder)
fig = tightsubplot(3, 2, [0 0], [0.072 0.009], [0.103 0.002]);
...
scatter(NB(:, 1), NB(:, 2), 500, 'r.', 'MarkerEdgeColor', hex2rgb('#de2d26'));
yyaxis right;
load('energy', 'E');
fill([E(:, 1)', fliplr(E(:, 1)')], [smooth(E(:, 2)-E(:, 3), 10)', fliplr(smooth(E(:, 2)+E(:, 3), 10)')], hex2rgb('#6baed6'), 'EdgeColor', 'none', 'FaceAlpha', 0.5);
scatter(E(:, 1), E(:, 2), 500, 'b.', 'MarkerEdgeColor', hex2rgb('#08519c'));
xticklabels({'', '0', '', '2', '', '4', ''});
set(gca, 'XTick', -1:5, 'FontSize', 24);
xlabel('log(t)', 'FontWeight', 'bold');
axis([-1.7 5.9 -4 4], 'square');
